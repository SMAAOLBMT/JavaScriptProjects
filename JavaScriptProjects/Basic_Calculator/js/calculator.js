...
function Input_Digit(digit){
    ...
    if (Wait_Second_Operand === true){
        ...
        Calculator.digit = Display_Value === '0' ? digit : Display_Value + digit;
    }
...
function Handle_Operator(Next_Operator){
    const { First_Operand, Display_Value, oprator } = Calculator;
    const Value_of_Input = parseFloat(Display_Value);
    //Check if an operator exists and if Wait_Second_Operand is true
    //Then updates the operator and exits the function
    if (oprator && Calculator.Wait_Second_Operand){
        Calculator.oprator = Next_Operator;
        return;
    }
...
//Resets the calculator when teh AC button is clicked
